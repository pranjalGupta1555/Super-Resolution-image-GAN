...
def lr_images(images_real , downscale):
    ...
    for img in  range(len(images_real)):
        resized = cv2.resize(images_real[img], dim, interpolation = cv2.INTER_AREA)
        images.append(imresize(resized, [resized.shape[0]//downscale,resized.shape[1]//downscale], interp='bicubic', mode=None))
    images_lr = array(images)
    #images_lr.reshape(96,96,3)
...
def plot_test_generated_images_for_model(output_dir, generator, x_test_lr , dim=(1, 3), figsize=(15, 5)):
    ...
    for index in range(examples):
        ...
        cv2.imwrite('./output2/lrimages/lrimage'+str(index)+'.jpg',cv2.cvtColor(image_batch_lr[index], cv2.COLOR_RGB2BGR))
        cv2.imwrite('./output2/srimages/srimage'+str(index)+'.jpg',cv2.cvtColor(generated_image[index], cv2.COLOR_RGB2BGR))
"""
        plt.figure(figsize=figsize)
        ...
        plt.axis('off')
        cv2.imwrite('./output2/lrimage.jpg',image_batch_lr[index])

        plt1.subplot(dim[0], dim[1], 2)
        plt1.imshow(generated_image[index], interpolation='nearest')
        plt1.axis('off')
        cv2.imwrite('./output2/srimage.jpg',generated_image[index])

        plt1.subplot(dim[0], dim[1], 3)
        plt1.imshow(image_batch_hr[index], interpolation='nearest')
        plt1.axis('off')
        cv2.imwrite('./output2/hrimage.jpg',image_batch_hr[index])

        plt1.tight_layout()
        plt1.savefig(output_dir + 'test_generated_image_%d.png' % index)
"""
        #plt.show()
...
def plot_test_generated_images(output_dir, generator, x_test_lr, file_names, figsize=(5, 5)):
    ...
    for index in range(examples):
        ...
        cv2.imwrite(output_dir+file_names[index]+'-super'+'.jpg',cv2.cvtColor(generated_image[index], cv2.COLOR_RGB2BGR))
        resized = cv2.resize(image_batch_lr[index], dim, interpolation = cv2.INTER_AREA)
        blurImg = cv2.blur(resized,(10,10))
        cv2.imwrite(output_dir+'manually_converted/'+file_names[index]+'-manual'+'.jpg',cv2.cvtColor(blurImg, cv2.COLOR_RGB2BGR))
        #plt.show()
